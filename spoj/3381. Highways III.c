...
{
    heap *node,**order=(heap **)calloc(n,sizeof(heap *)*n),*back,*store,*MainStore;
    node = back = (*H);
    ...
    if((*H)->child!=NULL)
    {
        ...
        do
        {
            ...
            node->left=(*H);
            node->parent=NULL;
            (*H)->right = (*H)->right->left = node;
            node=store;
        }while(node!=(*H)->child);
    }
    (*H)->right->left=(*H)->left;
    (*H)->left->right=(*H)->right;
    node = store = (*H) = (*H)->right;
    while(node!=(*H))
    {
        if(node->time<store->time)
            store = node;
        node=node->right;
    }
    (*H)=store;
